...
doesn't seem to belong to a particular familiy.
...
1. prefering tha latter definition
1. and supplies the library initializatino call as the one allowable 
...
+ *firsttime* -- a function which unsets itself on first execution,
  permintting, for example, one-time display of the copyright notice
  when sourceing a library.

+ *help* -- anything the author may want to say about the family

+ *init* -- may call *fam_iam* to initialze family sub-function names.
...
functions, files, or any other conventient class of objects.  The optional
behavior of a smart function is to execute its arugments as the command,
...
/copyright, firsttime, help, list, varialble/, and /vars/ may be
...
* references
+ this paper appears online at http://mcgowans.org/marty3/commonplace/software/librarydevelopment.html
+ [[../10_red/index.org][the Red Chapter]]
+ [[https://github.com/applemcg/shelf/blob/master/shelf.org][SHELF proposal]].
+ [[http://leanpub.com/ShellFunctions][Shell Functions]]
+ [[./backupfunction.org][The Only Backup You'll Ever Need]].
